...
import { Table, Input, Button, Space, Radio, Row, Col } from "antd";
import type {
  FilterDropdownProps,
  ExpandableConfig,
...
const DataTable = <T extends Record<string, any>>({
  ...
  return (
    <>
      <Row>
        <Col xs={12}>
          {statusFilterOptions.show && statusFilterOptions.filters.length && (
            <Radio.Group
              value={statusFilterValue}
              onChange={(e) => onStatusFilterChange(e.target.value)}
            >
              {statusFilterOptions.filters.map((filter) => (
                <Radio.Button key={filter} value={filter} className="capitalize">
                  {filter}
                </Radio.Button>
              ))}
            </Radio.Group>
