...
export const firebaseService = {
    ...
    postTags: async (
        ...
        try {
            const tagsDocument = (await getDoc(docRef)).data() as {
                tags: string[];
            };
    ...
    deleteTags: async (
        ...
        try {
            ...
            if (tagsDocument && tagsDocument.tags) {
                const payload = tagsDocument.tags.filter(
                    (tag) => !tags.includes(tag)
                );
    ...
    deleteData: async (
        ...
        try {
            ...
            const documentIdx = document.findIndex(
                (_doc: any) => _doc.uuid === uuid
            );
            if (documentIdx === -1) {
                message.error('No such document found!');
                return;
    ...
    getContactDetails: async (
        ...
        try {
            ...
            setLoading(false);
            message.error(
                'Something went wrong, cannot fetch contact details.'
            );
        }
    },
